...
def process_voice():
    ...
    prompt = f"""
    Analizza: "{testo_vocale}". 
    Rispondi SOLO JSON:
    {{"autista": "Nome", "targa": "TARGA", "partenza": "Luogo", "km": numero}}
    Targhe: piccolo->GA087CH, medio->GX942TS, grande->GG862HC.
    """
    headers = {"Authorization": f"Bearer {PERPLEXITY_API_KEY}", "Content-Type": "application/json"}
    payload = {
        "model": "llama-3.1-sonar-small-128k-online",
        "messages": [{"role": "system", "content": "Estrattore JSON dati furgoni."}, {"role": "user", "content": prompt}]
    }
    try:
        r = requests.post("https://api.perplexity.ai/chat/completions", headers=headers, json=payload)
        res = r.json()['choices'][0]['message']['content'].replace("```json", "").replace("```", "").strip()
        return json.loads(res)
    except:
        return {"error": "errore"}, 500
...
def index():
    ...
    if request.method == "POST":
        ...
        if azione == "start":
            ...
            # Logica PDF (semplificata per brevità)
            buffer = io.BytesIO()
            p = canvas.Canvas(buffer, pagesize=A4)
            p.drawString(100, 800, f"Viaggio {targa} - Autista {c['autista']}")
            p.save()
            buffer.seek(0)
