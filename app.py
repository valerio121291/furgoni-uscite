...
app = Flask(__name__)
app.secret_key = "csa_2026_final_pplx"
...
def chat_ia():
    try:
        testo_voce = request.get_json().get("messaggio", "")
        headers = {"Authorization": f"Bearer {PPLX_KEY}", "Content-Type": "application/json"}
        payload = {
            "model": "sonar",
            "messages": [
                {"role": "system", "content": "Sei un logistico. Rispondi SOLO JSON: {\"autista\": \"nome\", \"partenza\": \"città\", \"km\": 0, \"risposta\": \"breve\"}. Nomi: Valerio, Daniele, Costantino, Stefano."},
                {"role": "user", "content": testo_voce}
            ],
            "response_format": {"type": "json_object"}
        }
        r = requests.post("https://api.perplexity.ai/chat/completions", json=payload, headers=headers)
        # Se la chiave è sbagliata o non c'è credito, questo darà errore
        return r.json()['choices'][0]['message']['content']
    except Exception as e:
        return json.dumps({"risposta": f"Errore: {str(e)}"})
